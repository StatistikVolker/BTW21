  ...
  alphaend = 1)
{
  ...
  cartodata <- paste0("DataCartogram/Jena",party3,"by",cartoweight,"-",itermax,".Rdata")
  ...
  btw21 <- merge(wbzJ_spdf, BTW21JParteien %>% filter(Stimme == vote & Gruppenname %in% (party)), by.x="WBZ_NR", by.y="Gebietsnummer")%>% 
    st_as_sf() %>%
    st_transform(4326) %>% 
    as_Spatial()
  ...
  pal <- colorFactor(
    palette = c('grey25',alpha(prtycol, seq(alphastart, alphaend, length.out = lcollabs))),
  ...
  saveWidget(m, file=paste0("Jenamaps/map_btw21",party3,".html"))
  return(m)
